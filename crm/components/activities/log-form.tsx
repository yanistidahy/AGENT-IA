...
import { ACTIVITY_TYPES, type ActivityType } from "@/lib/domain/types";
...
interface LogFormProps {
  ...
  readonly onLogged: (createdTask: boolean) => void;
}
...
export function LogForm({ link, owners, defaultOwner, onCancel, onLogged }: LogFormProps) {
    ...
    >
      <div className="grid grid-cols-3 gap-2.5">
        <Field label="Type" errors={fields.type}>
          <select name="type" defaultValue="call" className={CONTROL}>
        ...
        <Field label="Date" errors={fields.date}>
          <input name="date" type="date" defaultValue={isoDay()} className={CONTROL} />
        </Field>
