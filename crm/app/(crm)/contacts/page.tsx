...
import { listCustomFilters } from "@/lib/api/custom-filters";
 ...
 *
 * **Un seul écran, et c'est tout le jalon 79.** Les groupes nommés du jalon 77
 * vivaient derrière leur propre entrée de rail et leur propre route ; ils sont
 * désormais des **filtres personnalisés** posés ici, à côté des puces de cycle
 * de vie, et combinables avec elles comme elles le sont entre elles. Un second
 * écran aurait fini par ne plus offrir les mêmes colonnes, et c'est toujours le
...
export default async function ContactsPage({
  ...
  /*
    **Un filtre personnalisé ouvre sur « tous les cycles de vie »**, sauf si
    l'URL en choisit un. C'est un choix fait à la main : en masquer les fiches
    closes ferait diverger le compte annoncé par la puce de ce que le tableau
    montre — l'écart que le jalon 49 a payé une fois. Un cycle explicitement
    choisi l'emporte, pour qu'on puisse croiser les deux, ce qui est la demande.
  */
  const base = parsed.success ? parsed.data : {};
  const query =
    base.filtre === undefined || base.filtre === ""
      ? base
      : { lifecycle: "all" as const, ...base };
  ...
  return (
    <ContactsView
      ...
      customFilters={customFilters.map((entry) => ({
        id: entry.id,
        name: entry.name,
